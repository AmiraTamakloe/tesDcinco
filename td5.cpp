...
void Item::afficherInfoPrimaireSur(ostream& os) const 
{
	os <<titre;
}

void Film::afficherInfoPrimaireSur(ostream& os) const 
{
	Item::afficherInfoPrimaireSur(os);
	os <<", par " << realisateur;
}

void Film::afficherLesActeurs(ostream& os) const 
{
	os << "Acteurs:" << endl;
...
void Livre::afficherInfoPrimaireSur(ostream& os) const 
{
	Item::afficherInfoPrimaireSur(os);
	os <<" de " << auteur ;
}

void FilmLivre::afficherInfoPrimaireSur(ostream& os) const
{

	Film::afficherInfoPrimaireSur(os);
	os<<", de " << auteur;
}
...
void afficherListeItems(T& listeItems)
{
	...
	for (auto&& item : listeItems) {
		item->afficherInfoPrimaireSur(cout);
		cout << "\n"<< ligneDeSeparation;
	}
}
...
int main(int argc, char* argv[])
{
	...
	cout << ligneDeSeparation;
	cout << "Voici la liste dans la forward_list : " << endl;
	forward_list<Item*> forwardListItems;
	auto it = forwardListItems.before_begin();
	for (auto&& elem : items) 
	{
		it = forwardListItems.emplace_after(it, elem.get());
	}
	afficherListeItems(forwardListItems);
	
	cout << ligneDeSeparation;
	cout << "Voici la liste inversé dans la forward_list :" << endl;
	forward_list<Item*> forwardListItemsRenverse;
	for (auto&& elements : forwardListItems)
	{
		forwardListItemsRenverse.emplace_front(elements);
	}
	afficherListeItems(forwardListItemsRenverse);
	cout << ligneDeSeparation;


	cout << "Voici une copie de la liste originale dans une autre forward_list :" << endl;
	forward_list<Item*> copieForwardListItems;
	auto copyIt = copieForwardListItems.before_begin();
	for (auto&& elems : forwardListItems)
	{
		copyIt = copieForwardListItems.emplace_after(copyIt, elems);
	}
	afficherListeItems(copieForwardListItems);

	cout << ligneDeSeparation;
	cout << "Voici une copie de la liste inverser dans un vector :" << endl;
	vector<Item*> copieRenverserVecteur;
	auto itVector = copieRenverserVecteur.begin();
	for (auto&& elems : forwardListItems)
	{
		itVector = copieRenverserVecteur.insert(itVector, elems);
	}
	afficherListeItems(copieRenverserVecteur);
	cout << ligneDeSeparation;
	cout << "Les acteurs du film Alien sont les suivants : " << endl;
	auto ptrAlienForwardList = forwardListItems.front();
	auto AlienForward = dynamic_cast<Film*>(ptrAlienForwardList);
	afficherActeurs(AlienForward);
	cout << ligneDeSeparation;

	cout << "Les items organisés en ordre alphabétique sont les suivants : " << endl; 
	auto comparateurValeur = [](Item* Item1, Item* Item2) {return Item1->titre < Item2->titre ;};
	set<Item*, decltype(comparateurValeur)> setElements;
		for (auto&& elem : items) 
		setElements.emplace(elem.get());
	afficherListeItems(setElements);

	unordered_map<string, Item*> unOrderedMapDItem;
	for (auto&& elem : items) {
		unOrderedMapDItem[elem->titre] = elem.get();
	}
	cout << *unOrderedMapDItem["The Hobbit"];

	cout << ligneDeSeparation;
	vector<Item*> vecteurAlgo;
	copy_if(forwardListItems.begin(), forwardListItems.end(), back_inserter(vecteurAlgo), [](Item* item) {return dynamic_cast<Film*>(item) != nullptr; });
	afficherListeItems(vecteurAlgo);
	cout << ligneDeSeparation;
	auto sommeRecette = transform_reduce(vecteurAlgo.begin(), vecteurAlgo.end(),0,plus{},[](Item* film){return dynamic_cast<Film*>(film)->recette;});
	cout << "La valeur totale des recettes est : " << sommeRecette << " M$";
}
